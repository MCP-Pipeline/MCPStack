...
class FastMCPConfigGenerator:
    ...
    def _get_command(command, stack) -> str:
        ...
        # Check OS environment first (for containers) then stack config
        os_env_command = os.environ.get("MCPSTACK_COMMAND")
        if os_env_command:
            return os_env_command
        env_command = stack.config.get_env_var("MCPSTACK_COMMAND")
        if env_command:
            return env_command
